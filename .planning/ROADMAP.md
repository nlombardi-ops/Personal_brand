...
- [x] 02-01-PLAN.md — Slice 1 (INGEST-01): client-upload to the private Blob store via a `handleUpload` token broker, the `Document` type + dual-mode store + file-type allow-list, the authenticated inline file proxy, and the `Documentos` surface with classify / retitle / date / archive

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 02-02-PLAN.md — Slice 2 (LOOP-02): the many-to-many `linked_loop_ids` link working in both directions, the widened `LoopDetailPanel` that resolves and opens a loop's attachments, upload-from-a-loop auto-attach, and the board's attachment-count chip
