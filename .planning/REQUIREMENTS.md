...
- [x] **LOOP-02**: User can attach one or more `Document`s (acta / contract / presupuesto / carta) to an OpenLoop and open them in the browser
- [x] **LOOP-03**: User can record multiple `Presupuesto`s on an obra-kind OpenLoop (provider, amount, scope, received/valid dates, document) and compare them side by side
