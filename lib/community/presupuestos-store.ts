...
const BLOB_PATHNAME = "community-presupuestos.json";
const LOCAL_PATH = join(process.cwd(), "data/community-presupuestos.json");
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    try {
      ...
      if (result) {
        ...
        return JSON.parse(text) as Presupuesto[];
      }
    } catch {
...
export async function savePresupuestos(presupuestos: Presupuesto[]): Promise<void> {
  const json = JSON.stringify(presupuestos, null, 2);
  if (process.env.BLOB_READ_WRITE_TOKEN) {
